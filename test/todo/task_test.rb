...
module Todo
  class TaskTest < Minitest::Test
    def setup
      @task_item = Task.new('do this and that')
    end

    def test_initialized_as_done
      task_item = Task.new('do this and that', true)
    ...
    def test_equality_is_true_when_description_and_doneness_are_eqaul
      other_task = Task.new(@task_item.formatted_description)
      assert_equal(other_task, @task_item)

      other_task = Task.new(@task_item.formatted_description, true)
      assert(other_task != @task_item)

      other_task = Task.new('different description')
      assert(other_task != @task_item)
    end

    def test_can_move_up_and_down
      last_task = Task.new('wash the car')
      list = TaskList.from_array([Task.new('do the dishes'), last_task])
      last_task.move_up
