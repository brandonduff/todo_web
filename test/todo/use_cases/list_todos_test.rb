  ...
  module UseCases
    class ListTodosTest < Minitest::Test
      ...
      def test_list_with_no_current_todos_returns_empty_list
        notepad = Notepad.create_null(current_day: @today)
        todos = ListTodos.new(notepad: notepad).perform
        assert_equal([], todos)
      end

      def test_list_returns_todos_as_array
        task_list = TaskListBuilder.new([{ description: 'hello' }, { description: 'goodbye' }]).build
        notepad = Notepad.create_null(current_day: @today, tasks: { @today => task_list })
      ...
      def test_all_options_returns_unfinished_tasks
        ...
        todos = ListTodos.new(all: true, notepad: notepad).perform
      ...
      def test_week_option
        ...
        notepad = Notepad.create_null(current_day: @today, tasks: { yesterday => first_task_list, @today => second_task_list })

        todos = ListTodos.new(week: true, notepad: notepad).perform
      ...
      def test_month_option
        ...
        second_task_list = TaskListBuilder.new([description: 'world']).build
        first_task_list = TaskListBuilder.new([description: 'hello']).build
        notepad = Notepad.create_null(current_day: @today, tasks: { last_week => first_task_list, @today => second_task_list })

        todos = ListTodos.new(month: true, notepad: notepad).perform
      ...
      def test_accepts_presenter
        ...
        todos = ListTodos.new(presenter: console_presenter, notepad: notepad).perform
